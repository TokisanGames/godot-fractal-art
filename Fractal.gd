...
func update_aspect_ratio() -> void:
	var ts:Vector2 = self.get_texture().get_size()
	var vs:Vector2 = get_viewport().get_size()
	scale = Vector2(vs.x/ts.x, vs.y/ts.y)
	position = Vector2(vs.x/2, vs.y/2)
	material.set_shader_param("aspect_ratio", vs.x/vs.y)
...
func _input(event) -> void:
	...
	if(event is InputEventMouseMotion):
		if(mouse_sets_seed && material.get_shader_param("seed")):
			var jseed:Vector2
			var mouse_speed:float = 0.25
			jseed = material.get_shader_param("seed")
			if(Input.is_key_pressed(KEY_SHIFT)): mouse_speed = 0.0833
			if(Input.is_key_pressed(KEY_CONTROL)): mouse_speed = 0.025
			jseed += mouse_speed * event.relative / (screen_size.y * material.get_shader_param("scale"))
			if(jseed.x==0.0 && jseed.y==0.0): jseed.y=.0001
			material.set_shader_param("seed", jseed)
		...
		if(mouse_sets_position):
			...
			move /= (screen_size.y * material.get_shader_param("scale"))
			move += material.get_shader_param("position")
			material.set_shader_param("position", move)
		...
		i = material.get_shader_param("scale") * 1.1
		material.set_shader_param("scale", i)
		...
		var fractal_pos:Vector2 = material.get_shader_param("position")
		var fractal_scale:float = material.get_shader_param("scale")
		fractal_pos -= mouse_pos/fractal_scale/10
		material.set_shader_param("position", fractal_pos)
		...
		if(material.get_shader_param("seed")):
		...
		i = material.get_shader_param("power") - 1
		if(i==0): i=-1
		material.set_shader_param("power", i)
		...
		i = material.get_shader_param("iterations") - 1
		if(i>=2):
			material.set_shader_param("iterations", i)
		...
		var smoothing = material.get_shader_param("smoothing")
		material.set_shader_param("smoothing", !smoothing)
		UI.find_node("Smoothing").pressed = material.get_shader_param("smoothing")
